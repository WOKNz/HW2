...
if __name__ == '__main__':
    ...
    # Generate random Points and then Segments
    PointsList = []
    for i in range(1, 50):
    ...
    for i in range(0, len(PointsList), 2):
        ...
        temparray.append([random.uniform(0.3, 1), random.uniform(0.3, 1), random.uniform(0.3, 1), random.uniform(0.3, 1)])
    ...
    while not len(Q) == 0:
        if Q[0][1] == 0:
            ...
            T.append(Q[0][0].segment)
            del Q[0]
            T.sort(key=lambda segment: segment.y_val(temp.x+0.0001))
